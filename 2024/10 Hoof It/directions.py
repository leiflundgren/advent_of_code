...
class Direction:
    ...
    def to_arrow(self):
        if self == Direction.N: return  '^'
        elif self == Direction.E: return '>'
        elif self == Direction.S:return  'v'
        elif self == Direction.W:return  '<'
        else:
...
# 
def direction(p1:tuple[int,int], p2:tuple[int,int]) -> Direction:
    (x1, y1) = p1
    (x2, y2) = p2
    if y1 != y2:
